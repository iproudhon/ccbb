...
(async () => {
  ...
  const box = () => page.evaluate('document.querySelector(".input-box").getBoundingClientRect().height');
  console.log("box before", await box());
  await page.evaluate('document.querySelector(".muxv .exp-btn").click()');
  await sleep(400);
  console.log("box maxed ", await box(), "log", await page.evaluate('document.querySelector(".mx-log").getBoundingClientRect().height'));
  await page.evaluate('document.querySelector(".muxv .exp-btn").click()');
  await sleep(400);
  console.log("box after ", await box());
  console.log("send bg", await page.evaluate('getComputedStyle(document.querySelector(".muxv .send-btn")).backgroundColor'));
  console.log("foot", await page.evaluate('document.querySelector(".muxv .sv-foot").innerHTML'));
  process.exit(0);
